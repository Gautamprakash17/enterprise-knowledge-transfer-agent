...
def test_load_txt(tmp_path):
    ...
    assert doc.doc_id
    assert doc.doc_id.startswith("file:")


def test_same_content_same_doc_id_across_paths(tmp_path):
    # Simulate "same file uploaded from different systems" (different paths).
    a_dir = tmp_path / "sys_a"
    b_dir = tmp_path / "sys_b"
    a_dir.mkdir()
    b_dir.mkdir()
    (a_dir / "abc.txt").write_text("SAME CONTENT")
    (b_dir / "abc.txt").write_text("SAME CONTENT")

    a = load_txt(a_dir / "abc.txt")
    b = load_txt(b_dir / "abc.txt")
    assert a is not None and b is not None
    assert a.doc_id == b.doc_id


def test_changed_content_changes_doc_id(tmp_path):
    f1 = tmp_path / "a.txt"
    f2 = tmp_path / "b.txt"
    f1.write_text("hello")
    f2.write_text("hello!!!")
