...
def _git_clone_or_pull(remote_url: str, token: Optional[str]) -> Path:
    """
    ...
    branch: list[str] = []
    if settings.github_clone_branch:
        branch = ["-b", settings.github_clone_branch]

    cmd = ["git", "clone", *shallow, *branch, auth_url, str(dest)]
...
def _resolve_repo_ref_to_path(repo_ref: str, token: Optional[str]) -> Path:
    """Local path as-is; remote URL -> clone cache path."""
    ref = repo_ref.strip()
    if not ref:
        raise ValueError("empty repo ref")
    if _is_remote_git_url(ref):
        return _git_clone_or_pull(ref, token)
